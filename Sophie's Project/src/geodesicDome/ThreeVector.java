...
public class ThreeVector {
	...
	// between two vectors.
	public static double scalarProduct(ThreeVector vecA, ThreeVector vecB) {
		return (vecA.pr_x * vecB.pr_x + vecA.pr_y * vecB.pr_y + vecA.pr_z * vecB.pr_z);
	}

	// Create a static method that returns a ThreeVector of the
	// vector product between two vectors.
	public static ThreeVector vectorProduct(ThreeVector vecA, ThreeVector vecB) {
		double vP_x = (vecA.pr_x * vecB.pr_y) - (vecA.pr_y * vecB.pr_x);
		double vP_y = (vecA.pr_y * vecB.pr_z) - (vecA.pr_z * vecB.pr_y);
		double vP_z = (vecA.pr_z * vecB.pr_x) - (vecA.pr_x * vecB.pr_z);
		return new ThreeVector(vP_x, vP_y, vP_z);
	}

	// Static method to add two vectors and return a ThreeVector
	public static ThreeVector add(ThreeVector vecA, ThreeVector vecB) {
		return new ThreeVector(vecA.pr_x + vecB.pr_x, vecA.pr_y + vecB.pr_y, vecA.pr_z + vecB.pr_z);
	}

	// Static method to subtract two vectors and return a ThreeVector
	public static ThreeVector subtract(ThreeVector vecA, ThreeVector vecB) {
		return new ThreeVector(vecA.pr_x - vecB.pr_x, vecA.pr_y - vecB.pr_y, vecA.pr_z - vecB.pr_z);
	}
	...
	public static double angle(ThreeVector vecA, ThreeVector vecB) {
		...
		double magsP = Math.abs(sP);
		double magA = vecA.magnitude();
		double magB = vecB.magnitude();
		return Math.acos(magsP / (magA * magB));
	}
